    ...
    <div class="row">
      ...
      <?php 
        include "include/config.php";
             ...
             $sql1 = "SELECT * FROM `user-table` Where `user_id`  = '{$author_id}'";
             $result1 = mysqli_query($con,$sql1);
                $row1 =  mysqli_fetch_assoc($result1);
      ?>
      <h3>POSTED BY : <?php echo $row1 ['name']?></h3>
      <hr>
      <?php 
        include "include/config.php";

        $author_id1 = $_GET['author_id'];

        $sql = "SELECT `post`.`post_img` , `post`.`post_id` , `post`.`title` , `post`.`description`,`user-table`.`name`,`category`.`category_name`, `post`.`post_date` , `post`.`author` FROM `post`
        LEFT JOIN category ON `post`.`category` = `category`.`cat_id` 
        LEFT JOIN `user-table` ON `post`.`author` = `user-table`.`user_id` WHERE
          `user_id` = '{$author_id1}' ORDER BY `post`.`post_id` DESC ;";
